...
app.get("/posts/:id", (req, res)=>{
    ...
    const sql =`select id, title, content, author, createdAt, count from posts where id=?`;
...
app.put("/posts/:postId/comments/:id", (req, res)=>{
    ...
    if(result.changes){
        ...
        res.status(400).json({result:'ok', message:'Comment is not found'});
    }
});
...
//문제1)게시글 상세 GET /posts/1 을 요철할 경우 해당글의 comments에 답글이 있는 경우
...
app.get("/posts/test/:id", (req, res)=>{
    const id = req.params.id;
    const result = db.prepare(`select a.id, a.title, a.content, a.author, a.createdAt
        ,(select content from comments where postId = a.id ) as comment_content 
        from posts a where a.id = ?`).all(id);
